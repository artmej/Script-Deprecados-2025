...
#Re.PARAMETER ValidateScenarioOnly
    Solo valida el escenario sin realizar la migración.

.PARAMETER Force
    Omite las confirmaciones de usuario.Modules Az.Network, Az.Resources
...
try {
    ...
    # Paso 6: Confirmación
    if (-not $Force -and -not $WhatIfPreference -and -not $ValidateScenarioOnly) {
        Write-Log ""
